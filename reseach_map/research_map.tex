...
\begin{itemize}
  ...
  \item 予測ターゲット：2-back 条件の平均正答率 $y^{\text{acc}}$ と
        中央値反応時間 $y^{\text{rt}}$ の 2 変数
\end{itemize}
...
\noindent\underline{Split}（毎 5 ラウンド）：バブル内の最大距離が閾値を超えたら2分割する：
\begin{align}
  d_{\max}(B) &= \max_{i,\,j \in B}\, d\!\left(\bar{\bm{h}}_i,\,\bar{\bm{h}}_j\right)
  \\
  \text{if }\; d_{\max}(B) &> \theta_{\text{split}}:
  \notag\\
  (i^*,\,j^*) &= \operatorname*{arg\,max}_{i,\,j \in B}\, d\!\left(\bar{\bm{h}}_i,\,\bar{\bm{h}}_j\right)
  \notag\\
  B_A &= \left\{k \in B : d\!\left(\bar{\bm{h}}_k,\,\bar{\bm{h}}_{i^*}\right)
                       \leq d\!\left(\bar{\bm{h}}_k,\,\bar{\bm{h}}_{j^*}\right)\right\},
  \quad B_B = B \setminus B_A
  \notag\\
  \theta_{\text{split}} &\leftarrow r_{\text{anneal}} \cdot \theta_{\text{split}}
...
\begin{align}
  ...
  \\
  \bm{h}_{\text{ret},i}
  &= \frac{1}{k} \sum_{j \in \mathcal{N}_i} \bar{\bm{h}}_j^*
  \in \mathbb{R}^{128}
\end{align}
